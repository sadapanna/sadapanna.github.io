...
2. **Beats** — drag across the rendered page (canvas-native selection via the layout map) and click *Add Beat*. Each beat = char range + highlight effect + auto-framed camera pan/zoom + timings (move-in / highlight / hold, with separate camera & highlight easing). Reorder by drag, edit timings inline. *Track words* (per beat, or as a default for new beats): instead of framing the whole span statically, the camera follows the highlight's leading point at a tighter zoom during the highlight — like an eye reading along — then eases back out to the full-span framing during the hold. The focus path is precomputed per beat (leading point per effect: revealing word, typewriter caret, or sweep edge along the span, smoothed so line wraps become quick pans), so it stays a pure function of `t` and exports stay frame-identical. *Tap-sync*: play your narration and tap Space per beat. *Fit to N seconds* scales all timings proportionally.
